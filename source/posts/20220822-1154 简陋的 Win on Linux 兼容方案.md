...
相比于 NTFS 压缩，QCOW2 的压缩解压完全在宿主机上运行，性能损失较小，且 ZSTD 算法很好地平衡了速度与体积。但这仅能压缩镜像已有内容，对新写入的数据依旧不作压缩。另外，QEMU 默认的 ZSTD 压缩配置是默认等级。由于此镜像只需读取无需写入，所以可以 [修改 QCOW2 镜像的 ZSTD 压缩级别](/./post/202206112353)，尽力压到最小。
...
```
mkisofs -joliet-long -o share.iso ./share
...
-vga none -device bochs-display,xres=1920,yres=1080,refresh_rate=30,vgamem=134217728 -display sdl,gl=on
